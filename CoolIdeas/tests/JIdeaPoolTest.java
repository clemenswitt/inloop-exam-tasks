...
public class JIdeaPoolTest {
    ...
    public void testAddJIdeaJTopic() {
        ...
        if (ideasInTopic.size() != 1 || ideasInTopic.get(0) != i1) {
            fail("JIdeaPool.add(JIdea, JTopic) should not add an idea if the pool already contains another unidentical"
        ...
        assertTrue("JIdeaPool.add(JIdea, JTopic) should add the idea to the set of ideas of the given topic even if"
                + " an identical idea is already associated to another topic!", actPool.containsKey(t2)
